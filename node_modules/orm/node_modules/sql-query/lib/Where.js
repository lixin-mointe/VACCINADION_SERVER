...
function buildOrGroup(Dialect, where, opts) {
	...
	for (var k in where.w) {
		...
		//if ([ "eq", "lt", "like", "lte" ].indexOf(k) >= 0) {
		//op = k;
		
	// 	console.log ("666666666typeof where.w[k].sql_comparator:  "+typeof where.w[k].sql_comparator);
		if (typeof where.w[k].sql_comparator == "function") {
			op = where.w[k].sql_comparator();
			//console.log ("------------");
			//for(var i in where.w[k]){
			//	console.log("i:"+i+" []:"+where.w[k][i]);
			//}
			//console.log ("k:"+k+"op")+op;
			//console.log ("===========");
			switch (op) {
				case "between":
					query.push(
	...
	//console.log("query.join-----------:"+query.join(" AND "));
